...
import { VideoIcon, AlertTriangleIcon, XIcon, ArrowLeftIcon } from 'lucide-react';
export function VolunteerSurveillance() {
  ...
  return <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {cameras.map((camera, index) => <button key={camera.id} onClick={() => handleCameraClick(camera.id)} className={`relative bg-slate-900 border rounded-xl overflow-hidden transition-all w-full ${selectedCameras.includes(camera.id) ? 'border-green-500 ring-2 ring-green-500' : 'border-slate-800 hover:border-green-500'}`}>
            <div className="aspect-video bg-slate-950 relative">
              ...
              {selectedCameras.includes(camera.id) && <div className="absolute inset-0 bg-green-500/20 flex items-center justify-center">
